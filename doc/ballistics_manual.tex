...
{\large Version 3.0 (Consolidated) --- \today}
\end{titlepage}
...
$T$ & Temperature & \si{\kelvin} or $\degF$ \\
...
The atmosphere is the medium through which the bullet travels. Accurate modeling of air density, speed of sound, and viscosity is prerequisite to all exterior ballistic calculations.

\section{ICAO Standard Atmosphere}

The International Civil Aviation Organization (ICAO) standard atmosphere defines baseline conditions at sea level \citep{icao1993manual}:

\begin{align}
  T_0 &= \SI{288.15}{\kelvin} \quad (59\,\degF) \\
  P_0 &= \SI{101325}{\pascal} \quad (29.92\;\text{in Hg}) \\
  \rho_0 &= \SI{1.2250}{\kilogram\per\meter\cubed} \\
  a_0 &= \SI{340.294}{\meter\per\second} \quad (\SI{1116.45}{\fps}) \\
  H_0 &= 0\% \quad \text{(dry air)}
\end{align}

Default values: $T_0 = 288.15\;\text{K}$, $P_0 = 101325\;\text{Pa}$, $\rho_0 = 1.225\;\text{kg/m}^3$, $H_0 = 0\%$.
...
\end{equation}
where $g_0 = \SI{9.80665}{\meter\per\second\squared}$ and $R_{\mathrm{air}} = \SI{287.058}{\joule\per\kilogram\per\kelvin}$.
...
\begin{equation}
  \rho = \frac{P - 0.3783\,e}{R_{\mathrm{air}}\,T}
  \label{eq:rho_humid}
\end{equation}

The factor 0.3783 arises from $(1 - M_w/M_{\mathrm{air}}) = 1 - 18.015/28.964 \approx 0.3780$.
...
At the ICAO standard temperature: $a_0 = 20.0468\sqrt{288.15} = \SI{340.29}{\meter\per\second}$.

\section{Density Altitude}

Shooters often use density altitude (DA) as a single parameter that encapsulates the combined effects of temperature, pressure, and humidity on air density. DA is the altitude in the standard atmosphere that has the same density as the actual conditions:
\begin{equation}
  \text{DA} = \frac{T_0}{L}\left[1 - \left(\frac{\rho}{\rho_0}\right)^{1/(n-1)}\right]
  \label{eq:density_altitude}
\end{equation}
where $n = g_0/(L\,R_{\mathrm{air}}) + 1 = 6.2559$.
...
Interior ballistics governs the conversion of chemical energy stored in propellant into kinetic energy of the projectile. For the competition shooter, the key outputs are muzzle velocity and its consistency (extreme spread and standard deviation).

\section{The Thermodynamic Problem}

\subsection{Nobel--Abel Equation of State}

Propellant gases at high pressure deviate significantly from ideal-gas behavior. The Nobel--Abel equation accounts for the finite co-volume of gas molecules \citep{mccoy1999modern, carlucci2007ballistics}:
\begin{equation}
  P\,(V - \eta) = n\,R_u\,T_g
  \label{eq:nobel_abel}
\end{equation}
where $P$ is the chamber pressure, $V$ is the gas volume, $\eta = n\,b$ is the total co-volume, $n$ is the number of moles of gas, $R_u = \SI{8.3145}{\joule\per\mole\per\kelvin}$ is the universal gas constant, and $T_g$ is the gas temperature.
...
For small arms propellants, the empirical Vielle law is commonly used:
\begin{equation}
  r_b = \beta\,P^n
  \label{eq:vielle}
\end{equation}
...
\end{equation}
where $\sigma_T$ is the temperature sensitivity coefficient, typically $0.5$ to $2.0\;\text{fps}/\degF$ depending on propellant. Modern temperature-stable propellants like Hodgdon Extreme series have $\sigma_T \approx 0.5\;\text{fps}/\degF$ \citep{litz2011applied}.

\default{$\sigma_T = 1.0\;\text{fps}/\degF$}
...
Transitional (or intermediate) ballistics covers the brief period after the bullet exits the muzzle and before it settles into free flight. This phase lasts only a few calibers of travel but establishes critical initial conditions for the exterior trajectory.

\section{Muzzle Gas Dynamics}

When the bullet exits the barrel, the high-pressure propellant gases (\SI{10}{}--\SI{70}{\mega\pascal}) expand rapidly into the atmosphere. The gas jet initially travels faster than the bullet and forms a complex shock-wave structure \citep{carlucci2007ballistics}:

\begin{enumerate}
  \item \textbf{Muzzle blast wave}: A spherical shock radiating outward.
  \item \textbf{Precursor blast}: From air compressed ahead of the bullet in the barrel.
  \item \textbf{Bottle shock}: The characteristic diamond-pattern Mach disk structure formed by the underexpanded supersonic gas jet.
\end{enumerate}

\section{Muzzle Tip-Off and Initial Yaw}

As the bullet exits the muzzle, asymmetric gas forces can impart a small angular velocity (tip-off rate). The resulting initial yaw angle $\alpha_0$ is typically small ($0.5$--$3\,\degF$) for well-made ammunition but is a significant factor in group size.

\section{Base Drag Transition}

Inside the barrel, the bullet base is subject to gas pressure. Upon exit, this pressure rapidly drops to ambient, and base drag increases significantly. The base drag coefficient transitions from a suppressed value to its free-flight value over a distance of approximately 5--20 calibers from the muzzle \citep{mccoy1999modern}.

\section{Crown Effects}

The muzzle crown---the precise geometry at the barrel's end---is critical to accuracy because it determines how symmetrically the gases release around the bullet base.
...
This chapter develops the mathematical framework for predicting the bullet's trajectory in free flight.

\section{Vacuum Trajectory (Baseline)}

In the absence of air resistance, the trajectory is a simple parabola. This model is useful as a theoretical baseline but is \emph{insufficient for real-world ballistics} because bullets travel at supersonic speeds where aerodynamic drag dominates the behavior. Precision rifle shooting relies on understanding the physics governing projectile motion beyond the idealized vacuum case, accounting for gravity, drag, wind, and spin.

The equations of motion in vacuum are:
\begin{align}
  x(t) &= v_0 \cos\theta \cdot t \label{eq:vac_x}\\
  y(t) &= v_0 \sin\theta \cdot t - \frac{1}{2}g\,t^2 \label{eq:vac_y}
\end{align}

Eliminating $t$ from Eq.~(\ref{eq:vac_x}) and substituting into Eq.~(\ref{eq:vac_y}) yields the trajectory equation in terms of horizontal distance $x$:
\begin{equation}
...
Substituting this into Eq.~(\ref{eq:1d_v_t}) yields the velocity as a function of distance:
\begin{equation}
  \boxed{v(x) = v_0 e^{-kx}}
  \label{eq:1d_v_x}
\end{equation}
...
\midrule
0.00 & 0.1198 & 1.00 & 0.4002 & 2.00 & 0.2487 \\
0.50 & 0.1198 & 1.05 & 0.3810 & 2.50 & 0.2169 \\
0.80 & 0.1280 & 1.10 & 0.3621 & 3.00 & 0.1966 \\
0.90 & 0.1687 & 1.20 & 0.3278 & 3.50 & 0.1800 \\
0.95 & 0.2300 & 1.50 & 0.2860 & 4.00 & 0.1668 \\
\bottomrule
\end{tabular}
\end{table}
...
The companion Julia implementation (Appendix~\ref{app:julia_file}) provides both linear and cubic spline interpolation via the \texttt{DragModels} module.
...
Substituting $\Cd = i \cdot \Cd^{\mathrm{std}}$ and $\Bc = m/(i\,d^2)$:
\begin{equation}
...
Since the system of equations (Eqs.~\ref{eq:3dof_x}--\ref{eq:3dof_z}) has no closed-form solution due to the non-linear dependence of $\Cd$ on velocity, it must be solved numerically.
...
A typical time step of $h = 0.0005$ to $\SI{0.001}{\second}$ is sufficient.
...
\end{equation}
where $v_{\mathrm{avg}}$ is the average velocity over the range $R$. This model assumes the bullet instantly acquires the transverse velocity of the wind, which is physically unrealistic for high-velocity projectiles.

\subsection{Lag Rule (Quick Estimation)}

The Didion approximation, also known as the Lag Rule \citep{mccoy1999modern, litz2011applied}:
\begin{equation}
  \Delta z_{\mathrm{wind}} \approx W\sin\alpha_w \cdot \left(t_{\mathrm{actual}} - \frac{R}{v_0\cos\theta}\right)
  \label{eq:lag_rule}
\end{equation}

This is remarkably accurate (within 5\%) and provides an intuitive physical picture: the wind deflection equals the crosswind speed multiplied by the lag time that drag has cost the bullet.
...
\begin{equation}
  ...
  \label{eq:spin_drift}
\end{equation}
where $\Delta z_{\mathrm{spin}}$ is the deflection in inches, $S_g$ is the gyroscopic stability factor, and $t$ is the time of flight in seconds.

\subsection{Gyroscopic Stability Factor}

The Miller stability formula \citep{miller2005twist}:
\begin{equation}
  \boxed{S_g \approx \frac{30\,m}{T_w^2\,d^3\,l\,\left(1 + l^2\right)} \cdot \frac{v_0}{2800} \cdot \frac{T_0}{T}}
  \label{eq:miller_sg}
\end{equation}
where $m$ is bullet mass in grains, $T_w$ is twist rate in calibers per turn, $d$ is caliber in inches, $l$ is bullet length in calibers, $T_0 = 518.67\;\text{R}$ ($59\,\degF$).

For adequate stability, $S_g > 1.0$; values of $1.3$--$2.0$ are typical for match bullets.
...
\begin{equation}
  ...
  \label{eq:coriolis}
\end{equation}
where $\omega_E = \SI{7.2921e-5}{\radian\per\second}$.
...
The complete 3-DOF ballistic solver is implemented in the companion file \texttt{CompetitionBallistics.jl} (see Appendix~\ref{app:julia_file}). The solver includes: G1/G7 drag model interpolation from BRL tables, full RK4 integration of the 3-DOF equations, automatic zero-angle finding, Coriolis and E\"otv\"os corrections, post-hoc spin drift, and wind deflection. The \texttt{ShotParameters} struct accepts all physical inputs (bullet mass, BC, muzzle velocity, atmospheric conditions, wind, latitude, twist rate, etc.) and the \texttt{solve\_trajectory()} function returns a vector of \texttt{TrajectoryPoint} records containing time, position, velocity, Mach number, and kinetic energy at each integration step.

\begin{lstlisting}[caption={Example: running the solver},label={lst:solver_example}]
...
The 3-DOF point-mass model tracks only translational motion and uses empirical patches for spin drift, Magnus force, and aerodynamic jump. The 6-DOF model treats the bullet as a spinning rigid body with three translational and three rotational degrees of freedom, predicting yaw, precession, nutation, spin decay, and all gyroscopic phenomena from first principles. This chapter develops the full 6-DOF equations of motion and their Julia implementation.

\section{State Vector and Reference Frames}

\subsection{Coordinate Systems}

We require two frames:

\begin{itemize}[nosep]
  \item \textbf{Earth-fixed (inertial) frame} $\{O;\,\bm{e}_x, \bm{e}_y, \bm{e}_z\}$: $x$ downrange, $y$ up, $z$ right. The 3-DOF frame from Chapter~\ref{ch:exterior}.
  \item \textbf{Body-fixed frame} $\{G;\,\bm{b}_1, \bm{b}_2, \bm{b}_3\}$: Origin at the center of mass $G$. $\bm{b}_1$ along the symmetry axis (nose-forward), $\bm{b}_2$ and $\bm{b}_3$ in the transverse plane.
\end{itemize}

\subsection{Euler Angles and Orientation}

The orientation of the body frame relative to the inertial frame is described by three Euler angles in the aerospace (yaw-pitch-roll) convention:
...
For an axially symmetric projectile (body of revolution) with $I_x$ (axial) and $I_y = I_z$ (transverse), the inertia tensor is diagonal and Eq.~(\ref{eq:euler_tensor}) expands to the scalar Euler equations:
...
After each RK4 step, renormalize: $\bm{q} \leftarrow \bm{q}/\|\bm{q}\|$.
...
This is the exact form from which the Miller approximation (Eq.~\ref{eq:miller_sg}) is derived.
...
\begin{equation}
  1\;\text{MOA} = \frac{1}{60} \times \frac{\pi}{180} = \SI{2.9089e-4}{\radian}
  \label{eq:moa_rad}
\end{equation}

The common approximation: $1\;\text{MOA} \approx 1.047$ inches at \SI{100}{\yard}.

The angular correction in MOA for a vertical drop $\Delta y$ at range $R$ (assuming $\Delta y$ and $R$ are in the same units) is:
\begin{equation}
  \boxed{\text{MOA} = \frac{\Delta y}{R} \cdot \frac{180 \times 60}{\pi} \approx \frac{\Delta y}{R} \cdot 3437.75}
  \label{eq:moa_calc}
\end{equation}

\section{Milliradian (Mil)}

\begin{equation}
  1\;\text{mil} = \SI{0.001}{\radian} \approx 3.6\;\text{inches at \SI{100}{\yard}}
  \label{eq:mil}
\end{equation}

Conversion: $1\;\text{mil} = 3.438\;\text{MOA}$.

The angular correction in mils for a vertical drop $\Delta y$ at range $R$ (same units) is:
\begin{equation}
  \boxed{\text{mil} = \frac{\Delta y}{R} \times 1000}
  \label{eq:mil_calc}
\end{equation}
...
\textbf{Safety warning:} Always cross-reference with current published load data. Start 10\% below listed maximum charges and work up carefully.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Bullet length:} 1.10 in.
...
Using the Miller formula (Eq.~\ref{eq:miller_sg}):
\begin{align*}
  d &= 0.243\;\text{in.}, \quad l = 1.10/0.243 = 4.527\;\text{cal}, \quad T_w = 7.5/0.243 = 30.86\;\text{cal/turn} \\
  S_g &= \frac{30 \times 105}{30.86^2 \times 0.243^3 \times 4.527 \times (1 + 4.527^2)} \times \frac{2920}{2800} \times \frac{518.67}{518.67} \\
  &= \frac{3150}{30.86^2 \times 0.01435 \times 4.527 \times 21.49} \times 1.043 \\
  &\approx 1.68
\end{align*}

$S_g = 1.68 > 1.3$: well stabilized. The bullet will maintain gyroscopic stability down to approximately Mach 1.2 (about 1350\,fps), which occurs near 1100 yards for this load.

\subsection{Step 3: Atmospheric Conditions}

Match day conditions (summer F-Class match in Ottawa, Canada):
\begin{align*}
  T &= 82\,\degF = 300.9\;\text{K}, \quad P = 29.75\;\text{in Hg} = 100\,750\;\text{Pa} \\
  H &= 55\%, \quad \text{Altitude} = 250\;\text{ft}
\end{align*}

Air density from Eq.~(\ref{eq:rho_humid}):
\begin{align*}
...
Speed of sound: $a = 20.047\sqrt{300.9} = 347.7\;\text{m/s}$.
...
\begin{table}[H]
\centering
\caption{Computed trajectory: 6mm BR, 105 gr Berger Hybrid, $v_0 = 2920$ fps, 82$\,\degF$, 29.75 in Hg, 55\% RH. 8 mph wind at $60^{\circ}$. 100 yd zero. 1:7.5 RH twist, $45.4^{\circ}$N, firing West.}
...
\begin{table}[H]
\centering
\caption{DOPE card: 6mm BR 105 Berger / N140 / 2920 fps. Ottawa, 82$\,\degF$, 29.75 inHg. Wind hold is per 1 mph full crosswind.}
\label{tab:6br_dope}
\begin{tabular}{@{}rccc@{}}
\toprule
...
The dynamic stability factor $S_d$ must be considered alongside $S_g$ \citep{mccoy1999modern}. For full stability: $S_g > 1.0$ and $0 < S_d < 2$.
...
For any trajectory output $Y$ at range $R$:
\begin{equation}
...
This chapter bridges the mathematical models of the preceding chapters with the practical decisions a competition handloader must make at the reloading bench.

\section{The Reloader's Equation: What You Control}

From the 3-DOF equation of motion (Eq.~\ref{eq:3dof_vector}), the trajectory depends on three groups of quantities:
...
From Eq.~(\ref{eq:vertical_sd}), the vertical group size at range $R$ due to velocity variation:
\begin{equation}
...
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Propellant Class} & $\sigma_T$ \textbf{(fps/$\degF$)} & $\Delta v_0$ \textbf{for $\pm 40\,\degF$} \\
\midrule
Standard (IMR 4064, etc.) & 1.5--2.0 & $\pm$60--80 fps \\
Temperature-stable (Varget, H4350) & 0.3--0.8 & $\pm$12--32 fps \\
Highly stable (RL-16, N565) & 0.2--0.5 & $\pm$8--20 fps \\
\bottomrule
\end{tabular}
\end{table}
...
From the Le Duc model peak pressure (Eq.~\ref{eq:peak_pressure}):
\begin{equation}
...
\begin{enumerate}[label=\textbf{Step \arabic*:}]
  \item \textbf{Bullet selection.} Maximize $\Bc$. Verify $S_g > 1.4$ using Eq.~(\ref{eq:miller_sg}).
  \item \textbf{Propellant selection.} Temperature-stable. Calculate $\Delta v_0$ over competition temperature range using Eq.~(\ref{eq:temp_sensitivity}).
  ...
  \item \textbf{Temperature verification.} Compute come-up shift using Eq.~(\ref{eq:total_temp_shift}).
\end{enumerate}
...
Expressing in component form with $\vvec_{\mathrm{rel}} = \vvec - \vvec_w$ and dividing by $m$, introducing $\Cd\,A/(2m) = \pi\,\Cd^{\mathrm{std}}/(8\,\Bc)$, recovers Eqs.~(\ref{eq:3dof_x})--(\ref{eq:3dof_z}).
...
For lead-core jacketed bullets, the simplified form:
$T \approx 150\,d^2/l$.

Use $C = 150$ for $v < 2800$ fps, $C = 180$ for $v > 2800$ fps \citep{greenhill1879rotation}.

Stability guidance: $S_g < 1.0$ (unstable), $1.0$--$1.3$ (marginal), $1.3$--$2.0$ (optimal), $> 2.0$ (over-stabilized).


% ============================================================================
\begin{thebibliography}{99}

\bibitem[Bashforth(1870)]{bashforth1870description}
Bashforth, F. (1870). \emph{A Mathematical Treatise on the Motion of Projectiles}. Asher \& Co., London.

\bibitem[Braun(1958)]{braun1958aerodynamic}
Braun, W.F. (1958). The free flight aerodynamics range. \emph{Ballistic Research Laboratories Report No.\ 1048}. Aberdeen Proving Ground, MD.

\bibitem[Carlucci and Jacobson(2007)]{carlucci2007ballistics}
Carlucci, D.E. and Jacobson, S.S. (2007). \emph{Ballistics: Theory and Design of Guns and Ammunition}. CRC Press.

\bibitem[Greenhill(1879)]{greenhill1879rotation}
Greenhill, A.G. (1879). On the rotation required for the stability of an elongated projectile. \emph{Minutes of the Proceedings of the Royal Artillery Institution}, 10:577--593.

\bibitem[ICAO(1993)]{icao1993manual}
International Civil Aviation Organization (1993). \emph{Manual of the ICAO Standard Atmosphere}. ICAO Doc 7488/3, 3rd edition.

\bibitem[Ingalls(1893)]{ingalls1893exterior}
Ingalls, J.M. (1893). \emph{Exterior Ballistics in the Plane of Fire}. D.\ Van Nostrand Company.

\bibitem[Litz(2011)]{litz2011applied}
Litz, B. (2011). \emph{Applied Ballistics for Long Range Shooting}. Applied Ballistics, LLC, 2nd edition.

\bibitem[Litz(2015)]{litz2015ballistic}
Litz, B. (2015). \emph{Ballistic Performance of Rifle Bullets}. Applied Ballistics, LLC, 3rd edition.

\bibitem[McCoy(1999)]{mccoy1999modern}
McCoy, R.L. (1999). \emph{Modern Exterior Ballistics}. Schiffer Publishing, 2nd edition.

\bibitem[Miller(2005)]{miller2005twist}
Miller, D. (2005). The new twist rule. \emph{Precision Shooting}, March 2005.

\bibitem[Press et~al.(2007)]{press2007numerical}
Press, W.H. et~al. (2007). \emph{Numerical Recipes}. Cambridge University Press, 3rd edition.

\bibitem[Siacci(1888)]{siacci1888balistica}
Siacci, F. (1888). \emph{Balistica}. Arnaldo Forni Editore, Bologna.
