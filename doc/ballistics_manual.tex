...
\section{Motivation and Error Sensitivity}

The primary motivation for using a precise atmospheric model is the direct and measurable impact of air density on a projectile's trajectory. Aerodynamic drag is directly proportional to air density:
\begin{equation}
  F_{\text{drag}} = \frac{1}{2} \rho \, C_d \, A \, v^2
  \label{eq:drag_motivation}
\end{equation}
where $\rho$ is the air density, $C_d$ is the dimensionless drag coefficient, $A$ is the projectile's cross-sectional area, and $v$ is its velocity relative to the air. Consequently, a 10\% error in air density calculation results in approximately a 10\% error in drag, which directly affects both vertical drop and wind drift.

At long ranges, these effects are substantial. For example, at 1,000 meters, the difference between typical summer and winter conditions can exceed one meter (over 3 MOA) of vertical drop. By understanding and measuring these variables, a shooter can correct for environmental changes rather than relying on guesswork, ensuring first-round hits.

\section{ICAO Standard Atmosphere}

The International Civil Aviation Organization (ICAO) standard atmosphere defines baseline conditions at sea level \citep{icao1993manual,coesa1976standard}:

\begin{align}
  T_0 &= \SI{288.15}{\kelvin} \quad (\SI{15}{\celsius}, 59\,\degF) \\
...
As the bullet exits the muzzle, asymmetric gas forces can impart a small angular velocity (tip-off rate). The resulting initial yaw angle $\alpha_0$ is typically small ($0.5$--$3^{\circ}$) for well-made ammunition but is a significant factor in group size.
...
Eliminating $t$ from Eq.~\eqref{eq:vac_x} and substituting into Eq.~\eqref{eq:vac_y} yields the trajectory equation in terms of horizontal distance $x$:
\begin{equation}
...
\begin{align}
  ...
  \label{eq:rk4}
\end{align}
where $h$ is the time step, $\bm{y}_n$ is the state vector at time $t_n$, $\bm{f}$ is the vector of derivatives (velocities and accelerations), and $\bm{k}_1, \dots, \bm{k}_4$ are the intermediate derivative estimates.
...
\begin{equation}
  ...
  \label{eq:miller_sg}
\end{equation}
where $m$ is bullet mass in grains, $T_w$ is twist rate in calibers per turn, $d$ is caliber in inches, $l$ is bullet length in calibers, $v_0$ is muzzle velocity in fps, $T$ is ambient temperature, and $T_0 = 518.67\;\text{R}$ (\SI{15}{\celsius}, $59\,\degF$) is the standard reference temperature.
...
\bibitem[COESA(1976)]{coesa1976standard}
U.S.\ Committee on Extension to the Standard Atmosphere (1976). \emph{U.S.\ Standard Atmosphere, 1976}. NOAA--S/T~76-1562, NASA--TM--X--74335. National Oceanic and Atmospheric Administration, National Aeronautics and Space Administration, United States Air Force. \href{https://ntrs.nasa.gov/citations/19770009539}{NTRS 19770009539}.

\bibitem[ICAO(1993)]{icao1993manual}
International Civil Aviation Organization (1993). \emph{Manual of the ICAO Standard Atmosphere, extended to 80 kilometres (262\,500 feet)}. ICAO Doc 7488/3, 3rd edition. Identical to ISO 2533:1975/Add.~2:1997.
