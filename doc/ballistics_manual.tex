...
To account for this, we use the Nobel--Abel equation, which introduces an "excluded volume" or co-volume:
\begin{equation}
  P\,(V - \eta) = n\,R_u\,T_g
  \label{eq:nobel_abel}
\end{equation}
...
By subtracting the co-volume $\eta$ from the total volume $V$, the Nobel-Abel equation prevents the gas pressure from inaccurately dropping to zero.
...
When shooting at extreme long range, or when using modern high-BC bullets that border on being aerodynamically unstable, these fudge factors break down. We must upgrade to a Six-Degree-of-Freedom (6-DOF) model. 
...
\begin{itemize}[noitemsep]
    ...
    \item $S_g \in [1.0, 1.3]$: Marginally stable (will fly straight but lose ballistic coefficient).
    \item $S_g \in [1.3, 2.0]$: Optimal match stability.
    \item $S_g > 2.0$: Over-stabilized (can exaggerate spin drift and limit extreme long range precision).
\end{itemize}
