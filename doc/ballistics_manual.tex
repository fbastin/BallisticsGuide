...
Version 3.2 records that the 6-DOF is no longer a roadmap: it is implemented and validated against three published cases --- McCoy's .308\,in 168\,gr textbook example, the 105\,mm HE M1 shell, and the 5.56\,mm NATO family measured at the Ballistic Research Laboratory. This edition adds the moment-of-inertia estimator that feeds it (Section~\ref{sec:inertia}), the measured coefficient sets it can be run on, and the reasons a set may not be trusted. It also corrects an inclined-fire example that had drifted 10\,\% from what both implementations return, and qualifies the recommendation of spline interpolation, which is right for the dense standard drag tables and wrong for sparse measured ones.

Version 3.3 (2026-08-25) corrects the G7 ballistic coefficient of the Sierra 175\,gr MatchKing in the bullet data table (0.259 to 0.243, the value published by the sources that table cites) and the transonic ranges derived from it. It also adds Chapter~\ref{chap:dispersion}: the dispersion statistics, hit-probability model and Bayesian muzzle-velocity truing behind the analysis tools of the online calculator.
\end{abstract}
...
\addplot[thick, red!80!black] coordinates {
  (0,0.2629)(0.5,0.2032)(0.7,0.2165)(0.8,0.2546)(0.9,0.3415)(0.95,0.4084)
  (1.0,0.4805)(1.05,0.5427)(1.1,0.5883)(1.2,0.6393)(1.5,0.6573)
  (2.0,0.5934)(2.5,0.5397)(3.0,0.5133)};
...
\addplot[thick, blue!70!black] coordinates {
  ...
  (1.0,0.3803)(1.05,0.4043)(1.1,0.4014)(1.2,0.3884)(1.5,0.3440)
  (2.0,0.2980)(2.5,0.2697)(3.0,0.2424)};
...
from McCoy~\citep{mccoy1999modern} or from an equivalent published tabulation. The
...
It is critical to apply the RK4 integration to the \emph{full state vector} rather
than decoupling position and velocity (a common error that degrades long-range
accuracy). The drag term itself carries no free parameter: with $A = \pi d^2/4$ the
physical frontal area and $i = \mathrm{SD}/\Bc$ the form factor, both $d^2$ and $m$
cancel out of $a = \rho\,\Cd\,i\,A\,v^2/(2m)$, leaving the ballistic coefficient alone
in control of the trajectory---exactly as it should be.

\paragraph{A correction factor that should never have been there.} Until 2026-08-22
this section justified multiplying the deceleration by $4/\pi$, on the grounds that
the shooting definition of sectional density (referred to $d^2$) had to be bridged to
the BRL tables (referred to $\pi d^2/4$). The derivation above does not produce that
factor. What it actually did was compensate for supersonic drag-table values that were
wrong (see the warning in \texttt{G7\_RAW}), tuned so that one particular .308 load
arrived at 1000 yards near a plausible velocity. A global factor calibrated on a single
operating point is right at that point and wrong everywhere else. Both the factor and
the bad table data have been removed.
...
To ensure the physical integrity of our implementation, every trajectory in this
manual is checked nightly against an independent third-party point-mass solver
(\texttt{py-ballisticcalc}), on a frozen grid of fourteen cases covering three drag
models, three atmospheres, wind, spin drift and Coriolis. The comparison is a second
opinion, not an oracle: when it disagrees, the arbiter is published data (McCoy, Litz)
or measured DOPE.

Earlier editions of this section claimed cross-validation against JBM Ballistics. That
claim did not survive scrutiny --- the first genuine third-party comparison, on
2026-08-22, showed velocity errors of 6 to 9\,\% at 1000 yards --- and JBM withdrew its
online calculators in 2026 in any case. A validation claim that nothing re-checks is
worth exactly nothing; that is why the check is now automated and runs every night.
...
\subsection{Measured Sets: What Actually Exists}

The table above is a range of plausible magnitudes, not a coefficient set. A 6-DOF
run needs actual numbers, and the constraint is severe: the full set is measured in
a spark photography range and published for almost no commercial bullet. The
implementation therefore marks every set with a \texttt{provenance} field and warns
once per session when a solver runs on placeholder values --- a right solver fed
invented coefficients returns results that are \emph{credible and wrong}, which is a
worse failure than a wrong solver returning visible nonsense.

Seven projectiles are presently carried with a measured set, all from BRL spark-range
reports by R.~L. McCoy and all in the public domain:

\begin{table}[H]
\centering
\caption{Measured aerodynamic sets carried by the implementation}
\label{tab:measured_sets}
\begin{tabular}{@{}llcl@{}}
\toprule
\textbf{Projectile} & \textbf{Source} & \textbf{Mach range} & \textbf{Note} \\
\midrule
.308\,in 168\,gr Sierra Intl. & \citet{mccoy1999modern}, App.~A & 0.0--2.5 & Textbook case 9.1 \\
105\,mm HE M1 & \citet{mccoy1999modern}, App.~B & 0.0--2.5 & Artillery, case 9.2 \\
...
The 5.56\,mm sets are the first \emph{small-arms} Magnus and pitch-damping data the
library carries, and they cover the whole useful flight band of the cartridge. Their
tables are transcribed round by round, exactly as printed, and every curve the
library uses is derived from those rows at load time --- there is no second,
hand-copied copy that can drift out of step.

Three limits are recorded with them, and each is a trap for the unwary.

\emph{The tabulated $C_D$ is measured at the round's own yaw}, not at zero yaw. It is
reduced with $C_{D_0} = C_D - C_{D_{\alpha^2}}\sin^2\alpha_t$, taking
$C_{D_{\alpha^2}} = 3.5$; that coefficient is not measured by the report, but pairs
of rounds fired at the same Mach and different yaw bracket it between 3.4 and 5.
Keeping only the near-zero-yaw rounds instead leaves \emph{two} points for the M855,
both supersonic, and the curve then clamps a supersonic value straight across the
transonic peak.

\emph{These sets cannot produce a limit cycle.} Each $C_{M_{p\alpha}}$ value is a
single round at a single angle of attack, so the curve is a function of Mach only and
the yaw dependence is unresolved --- and it is precisely the sign change of the
Magnus moment with yaw that fixes limit-cycle amplitude. The scatter hints at the
behaviour (the M855 runs from $+0.10$ at $1.90^{\circ}$ to $-0.59$ at $2.87^{\circ}$)
but three points cannot separate a Mach effect from a yaw effect, and pretending
otherwise would be invention.

\emph{Sparse tables extrapolate silently.} A table lookup clamps at its ends, which
for nine rounds means answering confidently at a Mach nobody fired: the L110 tracer
has three Magnus points, all above $\Mach 1.89$. The implementation returns the
clamped value \emph{and warns once} outside the band actually fired, with 2\,\% of
slack so that an edge case does not train the reader to ignore the warning.

Physical characteristics --- mass, centre of gravity and both moments of inertia ---
are additionally carried for the four 5.56\,mm projectiles, the M118 Match, the
190\,gr Sierra MatchKing and the 168\,gr Sierra International (BRL-MR-3733, 1988),
and for four .22\,LR match bullets (BRL-MR-3877, 1990). The last of these covers a
regime none of the others do: entirely subsonic flight, $\Mach 0.77$ to $1.03$.

\paragraph{Validation.} Feeding the measured coefficients of BRL-MR-3476 into the
linearised theory of Section~\ref{sec:stability6dof} reproduces the
gyroscopic stability factors that the same report publishes from the same firings, to
3.5\,\% for the SS-109 and 5.3\,\% for the M855 (median 1.3\,\% and 0.5\,\%). The
test barrel's twist is not published; the published $S_g$ implies $7.01''$ and
$7.18''$, which is the $1{:}7''$ of the M16A2 and M249 that the report discusses in
its dispersion section --- an independent check that the set and the theory are
mutually consistent.
...
\subsection{Why the Solid-Cylinder Form Is Not Good Enough}

Equations~\eqref{eq:Ix} and~\eqref{eq:Iy} treat the bullet as a solid cylinder of
uniform density. Against the measured inertias of the .308\,in 168\,gr Sierra
International (McCoy's Table 9.2) that approximation returns $I_x$ high by
15.2\,\%, $I_y$ high by 71.2\,\% and the ratio $I_y/I_x$ high by 48.7\,\%. Since
$S_g \propto I_x^2/I_y$, the error propagates to a gyroscopic stability factor of
1.32 where the published value is 1.70 --- a shortfall of 22\,\%, which is the
difference between declaring a load stable and declaring it marginal.

The cylinder is wrong for two independent reasons. It ignores the \emph{shape} ---
a real bullet is an ogive over a cylinder over a boat tail, not a cylinder --- and
it ignores the \emph{internal layout}, a gilding-metal jacket of nearly constant
radial thickness around a lead core that does not fill it.

\subsection{Integration over the Actual Contour}

The companion implementation (\texttt{bullet\_inertia}) integrates the solid of
revolution directly. The external contour is described in calibers by four numbers:
the ogive length as a fraction of total length, the meplat diameter, the boat-tail
length and the boat-tail half-angle. The ogive is taken \emph{tangent}, so its
radius follows from the tangency condition
\begin{equation}
  R = \frac{L_n^2 + a^2}{2a}, \qquad a = \frac{d - d_{\mathrm{meplat}}}{2}
  \label{eq:tangent_ogive}
\end{equation}
with $L_n$ the ogive length. For the jacketed-lead construction the body is a shell
of constant radial thickness with an open base, a lead core rising from that base,
and a cavity above it; the core height is solved by bisection so that the integrated
mass matches the bullet's actual weight. If no core height reproduces the weight ---
the bullet being denser than a solid-lead fill or lighter than the bare jacket ---
the routine falls back to a uniform density and says so.

Scored against the same reference bullet, given its published contour, this returns
$I_x$ to $-0.2\,\%$, $I_y$ to $+2.4\,\%$, the ratio to $+2.6\,\%$ and the centre of
gravity to $-0.5\,\%$; $S_g$ comes out at 1.693 against the published 1.70. On a
20\,mm steel cone-cylinder of exactly known geometry (McCoy's Example 12.1) the
integrator itself is good to $-0.5\,\%$ on $I_x$ and $-4.0\,\%$ on $I_y$, which
bounds the numerical error and shows that what remained wrong in the cylinder
approximation was the \emph{shape}, not the arithmetic.

\subsection{Default Contour, and What It Costs}

When no contour is supplied the defaults describe a modern boat-tailed match
bullet: ogive fraction 0.541, boat tail 0.782 calibers at $7.5^{\circ}$, meplat
0.22 calibers. The first three are medians of measured populations --- the ogive
fraction and boat-tail length over 94 boat-tailed match bullets from a published
manufacturer's dimension table, the boat-tail half-angle over 83 cones recorded by
handloaders as two diameters and a height, from which the angle follows as
$\arctan\!\bigl((d_{\mathrm{maj}} - d_{\mathrm{min}})/2h\bigr)$.

Flat-base bullets are a separate family, not a boat tail with zero length: they are
shorter ($L/d$ median 3.34 against 4.87) and carry proportionally longer ogives
(0.638 against 0.541). Applying the boat-tail defaults to a flat base biases $S_g$
by $-9.7\,\%$, always in the same direction.

Two cautions follow from the calibration and are worth stating plainly.

\emph{First, a default calibrated on one reference case is calibrated on its
idiosyncrasy.} An earlier default boat-tail length of 0.50 calibers matched the
Sierra International almost exactly --- and that bullet sits at the 2nd percentile
of the modern match population, whose median is 0.782. It therefore scored well on
the single case with published inertias while being wrong for the bullets the
library is actually used on. The present defaults deliberately score \emph{worse}
on that case ($-4.9\,\%$ on $S_g$ instead of $-0.5\,\%$) and better on the
population.

\emph{Second, the residual spread is irreducible by a better constant.} Against
each bullet's own measured contour the defaults leave a median absolute error of
3.8\,\% on $S_g$ for boat tails and 4.1\,\% for flat bases, with a third of the
population outside $\pm 5\,\%$. A linear rule in $L/d$ was fitted and rejected: it
bought 0.4 points of median error while widening the tails. What the spread wants
is the dimension of the individual bullet, not a cleverer average.

Of the four contour parameters only the meplat remains uncalibrated, no public
source publishing it. Sweeping it over its plausible range now moves $I_x$ by
1.9\,\%, $I_y$ by 3.2\,\% and $S_g$ by 0.9\,\% --- against 6.3 / 4.2 / 9.7\,\% when
the boat-tail angle was a guess as well.

\subsection{Constructions the Model Does Not Cover}

Two measured cases fall outside the jacketed-lead assumption and are useful as
declared limits rather than as failures.

The 5.56\,mm SS-109 and M855 carry a \emph{steel penetrator} ahead of a lead rear
core, so their mass is laid out in two materials. Given the published contour and
true weight, the estimator lands 4--5\,\% low on $I_x$, 6--9\,\% low on $I_y$ and
5--7\,\% low on the centre of gravity. That is the composite core showing.

The .22\,LR match bullets are unjacketed lead, \emph{heeled} --- a rear section of
reduced diameter --- with a round nose ending in a spherical cap rather than a
meplat. Neither the heel nor the round nose is expressible in the four contour
parameters, so no contour is offered for them; only their measured inertias are
tabulated.

In both cases the defaults happen to score better than the true contour. That is
compensation between two errors, not accuracy, and it must not be read as a reason
to prefer the defaults.
...
\textbf{Velocity retention.} The bullet arrives at 1000 yards at 1493 fps (Mach 1.31), still above the transonic threshold ($\approx$1340 fps), but with less margin than the older editions of this manual reported: the corrected drag tables cost roughly 100 fps at this range. The 6mm BR with a high-BC 105\,gr bullet remains supersonic to 1000 yards, and not much beyond.

\textbf{Total elevation.} From the 100-yard zero, the shooter dials 27.8 MOA of elevation (approximately 8.1 mils). This is competitive with the 6.5 Creedmoor (31.6 MOA for 140 gr, Table~\ref{tab:drop_65cm}) despite the lighter bullet, thanks to the higher muzzle velocity.

\textbf{Wind deflection.} The 8 mph wind at $60^{\circ}$ produces a crosswind component $W_{\mathrm{cross}} = 8 \sin 60^{\circ} = 6.93$ mph and a headwind component $W_{\mathrm{head}} = 8 \cos 60^{\circ} = 4.0$ mph. The total wind deflection at 1000 yards is 52.3 inches (5.0 MOA). For a full-value 10 mph crosswind, this would scale to approximately 75.5 inches --- about 3\% more than the 6.5 Creedmoor (73.5 in.) but comparable, illustrating that the 6mm BR's higher velocity partly compensates for the lower BC.

\textbf{Spin drift.} The 1:7.5 RH twist produces 9.0 inches of rightward drift at 1000 yards (0.86 MOA). This is a systematic bias that should be dialed into the scope or included in the wind hold.

\textbf{Coriolis and E\"otv\"os.} Firing West at $45.4^{\circ}$N latitude:
\begin{itemize}[nosep]
  \item Horizontal Coriolis: $\approx 2.4$ inches right (0.23 MOA), same direction as spin drift.
  \item E\"otv\"os (firing West): $\approx 2.4$ inches \emph{down} (the bullet loses the ``boost'' of the Earth's rotation), adding about 0.2 MOA to the required elevation.
\end{itemize}

\textbf{Total horizontal correction:} Wind 52.3 in.\ $+$ spin drift 9.0 in.\ $+$ Coriolis 2.4 in.\ $= 63.7$ inches total rightward deflection (6.1 MOA).
...
\begin{table}[H]
\centering
\caption{Transonic transition ranges for common competition cartridges at sea-level standard conditions, \emph{computed with the solver of Chapter~\ref{ch:solver}} rather than quoted. The transonic zone is defined as $0.9 \leq \Mach \leq 1.1$ ($\approx\SI{310}{}$--$\SI{380}{\meter\per\second}$). Ballistic coefficients from Table~\ref{tab:bullet_data} where listed, otherwise from \citet{litz2015ballistic}.}
\label{tab:transonic_ranges}
\small
\begin{tabular}{@{}llcccc@{}}
\toprule
\textbf{Cartridge} & \textbf{Bullet (gr)} & \textbf{$\Bc$ (G7)} & \textbf{$v_0$ (m/s)} & \textbf{Enters transonic (m)} & \textbf{Subsonic (m)} \\
\midrule
.223 Rem        & 77 SMK            & 0.191 & 838 & $\approx$690  & $\approx$750 \\
.308 Win        & 155 Lapua Scenar  & 0.237 & 860 & $\approx$890  & $\approx$970 \\
.308 Win        & 175 SMK           & 0.243 & 775 & $\approx$780  & $\approx$860 \\
6.5 Creedmoor   & 140 Berger Hybrid & 0.311 & 823 & $\approx$1090 & $\approx$1200 \\
.300 Win Mag    & 185 Berger Jugg   & 0.283 & 943 & $\approx$1210 & $\approx$1300 \\
.338 Lapua Mag  & 300 Berger Hybrid & 0.419 & 838 & $\approx$1510 & $\approx$1660 \\
\bottomrule
\end{tabular}
\end{table}
...
\SI{1000}{\yard}, the drop along the sight line falls from $-331.1$ inches (flat) to
$-285.2$ inches (uphill) and $-277.8$ inches (downhill).

These figures have moved twice. They were $-272$, $-233$ and $-228$ until 2026-08-14,
then $-300.6$, $-258.4$ and $-252.2$ once a $4/\pi$ factor was introduced on $\Cd$.
That factor was itself an error, compensating fabricated supersonic drag tables; both
were removed on 2026-08-22, and the values above come from the corrected solver. The
lesson is worth stating plainly: a global scale factor tuned to make one trajectory
look right will hide a data error indefinitely, because it leaves the shape of the
error untouched while moving its level.
...
% ============================================================================
% CHAPTER: DISPERSION, HIT PROBABILITY AND VELOCITY TRUING
% ============================================================================
\chapter{Dispersion, Hit Probability and Velocity Truing}
\label{chap:dispersion}

\section{Motivation: From Groups to Decisions}

The sensitivity coefficients of Chapter~\ref{ch:sensitivity} describe the weapon. This chapter adds the shooter and the conditions, and answers the two questions a competitor ultimately faces. First: given my measured dispersion and my uncertainty about the wind, what is my probability of hitting a target of a given size? Second: my chronograph and my impacts disagree about the muzzle velocity --- which one should I believe, and by how much?

Both questions have closed-form answers under a single assumption: the impact point is a bivariate normal random variable, independent on the horizontal and vertical axes, with the same standard deviation $\sigma$ on each axis. Almost every dispersion model used in the shooting sports reduces to this assumption, so it is worth deriving its consequences once, carefully.

\section{The Rayleigh Model of a Centered Group}

Let the impact coordinates $(x, y)$ be independent normal with mean zero and standard deviation $\sigma$ on each axis. The radial distance of an impact from the group center,
\begin{equation}
  R = \sqrt{x^2 + y^2},
\end{equation}
then follows the \textbf{Rayleigh distribution}, whose cumulative form gives the probability of landing inside a circle of radius $r$:
\begin{equation}
  P(R \le r) = 1 - \exp\!\left(-\frac{r^2}{2\sigma^2}\right).
  \label{eq:rayleigh}
\end{equation}

Two closed-form summaries follow directly:
\begin{equation}
  \mathrm{CEP}_{50} = \sigma\sqrt{2\ln 2} \approx 1.1774\,\sigma,
  \qquad
  \mathbb{E}[R] = \sigma\sqrt{\pi/2} \approx 1.2533\,\sigma,
\end{equation}
where $\mathrm{CEP}_{50}$ (Circular Error Probable) is the radius containing half of all impacts, and $\mathbb{E}[R]$ is the mean radius of the group.

\section{From Group Size to Sigma}

Shooters measure the \emph{extreme spread} (ES) of a group, not $\sigma$. For $n$ shots from the same bivariate normal, the expected extreme spread is a known multiple of $\sigma$:
\begin{equation}
  \mathbb{E}[\mathrm{ES}] = k_n\,\sigma,
  \qquad
  \sigma = \frac{\mathrm{ES}}{k_n}.
  \label{eq:es_to_sigma}
\end{equation}

\begin{table}[H]
\centering
\caption{Expected extreme spread of an $n$-shot group, in units of $\sigma$}
\label{tab:es_factors}
\begin{tabular}{@{}lcccccccc@{}}
\toprule
$n$ & 3 & 4 & 5 & 6 & 7 & 8 & 9 & 10 \\
\midrule
$k_n$ & 1.693 & 2.058 & 2.326 & 2.534 & 2.704 & 2.847 & 2.970 & 3.078 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Caution:} the ES of a single small group is a poor estimator --- its own variance is large, and it systematically underestimates $\sigma$. Average several groups before dividing.

\section{Composing Independent Variance Sources}

Wind-reading uncertainty behaves as an additional lateral dispersion. If the wind speed is uncertain by $\sigma_w$ (in speed units) and the solver's wind sensitivity at the target is $S_w$ (lateral displacement per speed unit, measured by the finite differences of Eq.~\eqref{eq:sensitivity}), then
\begin{equation}
  \sigma_{\text{total}}^2 = \sigma_{\text{disp}}^2 + \left(S_w\,\sigma_w\right)^2 + \cdots
  \label{eq:quadrature}
\end{equation}
Independent sources compose in \emph{quadrature}: variances add, standard deviations do not.

\section{Hit Probability on a Circular Target}

Inserting the target radius and the composed $\sigma_{\text{total}}$ into Eq.~\eqref{eq:rayleigh} gives the single-shot hit probability.

\paragraph{Worked example.} A .308\,in, 175\,gr at 600\,yd groups 20\,cm extreme spread on 5 shots: $\sigma_{\text{disp}} = 20/2.326 = 8.60$\,cm. The wind is known to $\pm 3$\,km/h; the solver's wind sensitivity at that range is 13.5\,cm per km/h, so $\sigma_{\text{wind}} = 40.5$\,cm. In quadrature, $\sigma_{\text{total}} = 41.4$\,cm. On a 30\,cm steel plate ($r = 15$\,cm), Eq.~\eqref{eq:rayleigh} gives $P = 6\,\%$ --- against $78\,\%$ with a perfectly known wind. The wind accounts for 96\,\% of the variance: at this range, progress comes from reading the wind, not from tighter groups.

\section{Bayesian Truing of the Muzzle Velocity}

The chronograph provides a prior: $v_0 \sim \mathcal{N}(\bar{v}_0, \sigma_{\text{chrono}}^2)$. The observation is the vertical offset $\Delta h$ (signed) of the impact relative to the predicted point at range $R$, with reading uncertainty $\sigma_h$. The forward sensitivity $\partial h/\partial v_0$ is measured by central finite differences on the solver, Eq.~\eqref{eq:sensitivity}. The velocity implied by the observation is
\begin{equation}
  \hat{v}_0 = v_0 + \frac{\Delta h}{\partial h/\partial v_0},
\end{equation}
with observation-equivalent uncertainty $\sigma_{\text{obs}} = \sigma_h / \left|\partial h/\partial v_0\right|$. The conjugate Gaussian update combines prior and observation by adding their precisions:
\begin{equation}
  v_0^{\text{post}} = \frac{\bar{v}_0/\sigma_{\text{chrono}}^2 + \hat{v}_0/\sigma_{\text{obs}}^2}{1/\sigma_{\text{chrono}}^2 + 1/\sigma_{\text{obs}}^2},
  \qquad
  \sigma^{\text{post}} = \left(\frac{1}{\sigma_{\text{chrono}}^2} + \frac{1}{\sigma_{\text{obs}}^2}\right)^{-1/2}.
  \label{eq:truing}
\end{equation}

\paragraph{Worked example.} A 6.5\,mm Creedmoor at 800\,m: chronograph $2700$\,fps $\pm 3$\,m/s, predicted drop $-5.885$\,m, observed impact $14.7$\,cm above it. The finite-difference slope is 0.0051\,m per fps, so the implied velocity is 2729\,fps --- the true value if the chronograph reads 30\,fps low. With a 2\,cm reading uncertainty ($\sigma_{\text{obs}} \approx 1.2$\,m/s), the posterior is $2725 \pm 1.1$\,m/s: the observation, being more precise than the chronograph, carries the update, and the shift of 2.5 chronograph-SD is a strong signal.

\paragraph{Assumptions and cautions.} The truing absorbs any BC error into the muzzle velocity --- a BC error mimics a velocity error, but \emph{differently at each distance}, which is why truing at two distances separates them only approximately. The range must be velocity-sensitive (never near the zero). One observation leaves the posterior SD at the reading-uncertainty floor. And a shift below one chronograph-SD is not a signal: keep the chronograph value.

\section{Implementation}

The online calculator applies these three analyses to the active load: the sensitivity of Eq.~\eqref{eq:sensitivity} is measured by finite differences on the validated solver, and the constants of this chapter (the $k_n$ table, 1.1774, $\sqrt{2\ln 10}$) are the ones the tools use.
